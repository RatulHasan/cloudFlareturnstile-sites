...
function cloudflare_turnstile_settings_link( $links ) {
    $settings_link = '<a href="options-general.php?page=cloudflare-turnstile">' . __( 'Settings', 'turnstile-for-cloudflare' ) . '</a>';
...
add_action( 'admin_menu', function () {
    add_options_page( 'Cloudflare Turnstile', 'Cloudflare Turnstile', 'manage_options', 'turnstile-for-cloudflare', 'cloudflare_turnstile_page' );
} );
...
function cloudflare_turnstile_page() {
    ?>
    <div class="wrap">
        <h2><?php esc_html_e( 'Cloudflare Turnstile', 'turnstile-for-cloudflare' ); ?></h2>
        <small>
            <?php
            /* translators: %1$s: Opening anchor tag, %2$s: Closing anchor tag */
            echo sprintf( esc_html__( 'Enter your Cloudflare Site Key and Secret Key to enable Cloudflare Turnstile. Or create one from %1$sCloudflare%2$s.', 'turnstile-for-cloudflare' ), '<a href="https://dash.cloudflare.com/sign-up?to=/:account/turnstile" target="_blank">', '</a>' );
            ?>
        </small>
        <form method="post" action="options.php">
            ...
            <table class="form-table form-table-wide">
                <tr>
                    <th scope="row"><label for="cloudflare_turnstile_enable"><?php esc_html_e( 'Enable Cloudflare Turnstile', 'turnstile-for-cloudflare' ); ?></label></th>
                    <td><input type="checkbox" id="cloudflare_turnstile_enable" name="cloudflare_turnstile_enable" value="1" <?php checked( 1, get_option( 'cloudflare_turnstile_enable' ), true ); ?>></td>
                </tr>
                <tr>
                    <th scope="row"><label for="cloudflare_site_key"><?php esc_html_e( 'Site Key', 'turnstile-for-cloudflare' ); ?></label></th>
                    <td><input type="text" id="cloudflare_site_key" name="cloudflare_site_key" value="<?php echo esc_attr( $sitekey ); ?>" class="regular-text"></td>
                </tr>
                <tr>
                    <th scope="row"><label for="cloudflare_secret_key"><?php esc_html_e( 'Secret Key', 'turnstile-for-cloudflare' ); ?></label></th>
                    <td><input type="text" id="cloudflare_secret_key" name="cloudflare_secret_key" value="<?php echo esc_attr( $secretkey ); ?>" class="regular-text"></td>
                </tr>
                <tr>
                    <th scope="row"><label for="cloudflare_turnstile_login_enable"><?php esc_html_e( 'Enable in Login Form', 'turnstile-for-cloudflare' ); ?></label></th>
                ...
                <tr>
                    <th scope="row"><label for="cloudflare_turnstile_comment_enable"><?php esc_html_e( 'Enable in Comment Form', 'turnstile-for-cloudflare' ); ?></label></th>
...
add_action( 'wp_head', function () {
    ...
    wp_enqueue_script( 'turnstile-for-cloudflare', 'https://challenges.cloudflare.com/turnstile/v0/api.js' );
} );
...
function getResponse_keys( $captcha ) {
    ...
    $response = wp_remote_post( $url_path, array(
        'body'    => $data,
        'headers' => array(
...
add_action( 'init', function () {
    ...
    add_action( 'wp_enqueue_scripts', function() {
        wp_enqueue_script( 'turnstile-for-cloudflare', 'https://challenges.cloudflare.com/turnstile/v0/api.js', [], null, true );
    });
